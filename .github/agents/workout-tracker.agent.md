...
- The sheet columns are listed in `COLONNE` in [Code.gs](../../Code.gs): Data, Gruppo muscolare, Esercizio, Sottocategoria, Ripetizioni, Set, Peso (kg), Mono, Esplosiva, Commento. Values are written by header title, and a missing column is inserted automatically next to its predecessor, so older sheets migrate without shifting existing rows.
